...
gulp.task("svg", function () {
  return src([source_folder + "/img/svg/icon-*.svg"])
    ...
    .pipe(src([source_folder + "/img/svg/icon-svgmin/*.svg"]))
    .pipe(
      svgstore({
        inlineSvg: true,
      })
...
gulp.task("svgedit", gulp.series("svg", "svgtohtml"));
/**BEFORE */
...
function fontsStyle(params) {
  let file_content = fs.readFileSync(
    source_folder + "/scss/_global/_fonts.scss"
  );
  if (file_content == "") {
    fs.writeFile(source_folder + "/scss/_global/_fonts.scss", "", cb);
